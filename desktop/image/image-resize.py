...
def resize(path, w, h, q=90, f='JPEG'):
    ...
    for imagePath in dirs:
        ...
        if os.path.isfile(fullImagePath):
            if(isImageFile(fullImagePath)):
                ...
                imResize = image.resize((w, h), Image.ANTIALIAS)
...
def resizeAspectFit(path, scaleRatio, q=90, f='JPEG'):
    dirs = os.listdir(path)
    print(1)
    for imagePath in dirs:
        fullImagePath = os.path.join(path, imagePath)
        ...
        if os.path.isfile(fullImagePath):
            ...
            if(isImageFile(fullImagePath)):
                ...
                imResize = image.resize((int(width * scaleRatio), int(height * scaleRatio)), Image.ANTIALIAS)
...
def applyResizing(args):
    ...
    if(args.targetScaleRatio != 1.0):
        ...
        resizeAspectFit(imagesDirPath, args.targetScaleRatio, args.quality, args.format)
    else:
        if(args.targetHeight < 1 and args.targetWidth < 1):
            ...
            resize(imagesDirPath, args.targetWidth, args.targetHeight, args.quality, args.format)
